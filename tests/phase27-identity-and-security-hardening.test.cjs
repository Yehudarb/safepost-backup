...
(async () => {
    ...
    try {
        ...
        console.log('\n G. identity bind is backend-authoritative');
        {
            ...
            const notRunning = await work(A, 'POST', `/scans/${scanId}/bind-identity`, { facebook_user_id: FB_ID_A });
            ...
            for (const bad of ['abc', '', '12', null, '1'.repeat(31), '10;drop', 1.5]) {
                const r = await work(A, 'POST', `/scans/${scanId}/bind-identity`, { facebook_user_id: bad });
            ...
            const bad = await work(A, 'POST', '/scans/not-a-uuid/bind-identity', { facebook_user_id: FB_ID_A });
            assert('a malformed scan id is refused', bad.status === 400 && bad.body?.error === 'Invalid id');

            const cross = await work(B, 'POST', `/scans/${scanId}/bind-identity`, { facebook_user_id: FB_ID_B });
            ...
            const ok = await work(A, 'POST', `/scans/${scanId}/bind-identity`, { facebook_user_id: FB_ID_A });
            ...
            const overwrite = await work(A, 'POST', `/scans/${scanId}/bind-identity`, { facebook_user_id: FB_ID_B });
            ...
            const repeat = await work(A, 'POST', `/scans/${scanId}/bind-identity`, { facebook_user_id: FB_ID_A });
