...
const StatusPill = ({ status, revoked }) => {
    ...
    return (
        <span className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full border text-[9px] font-bold uppercase tracking-wider ${map.c}`}>
...
export default function WorkersPanel({ api, onClose }) {
    ...
    const genCode = async () => {
        ...
        setCopied(false);
        try {
            setCode(await api.createPairingCode());
        } catch (e) {
            setErr(e.message);
        }
    };
    ...
    return (
        <div className="fixed inset-0 z-[150] bg-black/80 backdrop-blur-md flex items-center justify-center p-4 animate-in fade-in duration-300">
            <div className="bg-[#161b22] border border-[#30363d] w-full max-w-2xl max-h-[84vh] rounded-2xl shadow-2xl flex flex-col overflow-hidden">
                <div className="px-4 py-3 border-b border-[#30363d] flex justify-between items-center bg-[#1c2128]">
                    <div className="flex items-center gap-3 min-w-0">
                        <div className="p-2 bg-blue-500/10 rounded-lg border border-blue-500/20">
                            <MonitorSmartphone className="w-5 h-5 text-blue-400" />
                        </div>
                        <div className="min-w-0">
                            <h2 className="text-lg font-bold text-white truncate">Paired Devices</h2>
                            <p className="text-[9px] text-gray-500 uppercase tracking-widest font-black truncate">Chrome extensions linked to this workspace</p>
                        </div>
                    </div>
                ...
                <div className="p-4 overflow-y-auto bg-[#0d1117] space-y-4">
                    <div className="bg-[#161b22] border border-[#30363d] rounded-xl p-3">
                        <div className="flex items-center justify-between gap-3">
                            ...
                            <button onClick={genCode} className="px-3 py-1.5 bg-blue-600 hover:bg-blue-500 text-white text-[10px] font-bold rounded-lg transition">
                                Generate pairing code
                            </button>
                        </div>
                        {code && (
                            <div className="mt-3 flex items-center gap-2.5 flex-wrap">
                                <code className="px-3 py-2 rounded-lg bg-[#0d1117] border border-blue-500/30 text-xl font-mono font-black tracking-[0.22em] text-blue-300">{code.code}</code>
                                <button onClick={copyCode} className="p-2 rounded-lg bg-[#21262d] hover:bg-[#30363d] text-gray-300" title="Copy">
                                    {copied ? <Check className="w-4 h-4 text-emerald-400" /> : <Copy className="w-4 h-4" />}
                                </button>
                                <span className={`text-[11px] ${codeSecondsLeft > 0 ? 'text-gray-400' : 'text-rose-400'}`}>
                                    {codeSecondsLeft > 0 ? `expires in ${Math.floor(codeSecondsLeft / 60)}:${String(codeSecondsLeft % 60).padStart(2, '0')}` : 'expired'}
                                </span>
                            </div>
                        )}
                        <p className="mt-2 text-[10px] text-gray-500">Enter this code in the extension popup. Single-use, expires in 10 minutes.</p>
                    </div>
                    ...
                    {loading ? (
                        ...
                        <div className="space-y-2">
                            {workers.map((w) => (
                                <div key={w.id} className={`flex items-center justify-between p-2.5 rounded-xl border ${w.revoked_at ? 'border-rose-500/20 bg-rose-500/5 opacity-70' : 'border-[#30363d] bg-[#161b22]'}`}>
                                    <div className="min-w-0">
                                        <div className="flex items-center gap-2">
                                            <span className="text-[13px] font-bold text-white truncate">{w.worker_name}</span>
